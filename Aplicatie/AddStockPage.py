...
class AddStockPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.page = tk.Label(self, text="Adauga stock, bifeaza produs si introdu cantitatea adaugata")
        self.page.place(x=50, y=40, width=400, height=30)
        self.brandLabel = tk.Label(self, text="Marca")
        self.brandLabel.place(x=50, y=80, width = 100, height = 30)
        self.brands = ttk.Combobox(self)
        self.brands.place(x=170, y=80, width=150, height=30)
        self.brands.bind('<<ComboboxSelected>>', self.updateModels)
        self.modelLabel = tk.Label(self, text="Model")
        self.modelLabel.place(x=50, y=120, width = 100, height = 30)
        self.model = ttk.Combobox(self)
        self.model.place(x=170, y=120, width=150, height=30)
        self.categoryLabel = tk.Label(self, text="Categorie")
        self.categoryLabel.place(x=50, y=160, width = 100, height = 30)
        self.category = ttk.Combobox(self)
        self.category.place(x=170, y=160, width=150, height=30)
        self.search = tk.Button(self, text="Search", command=self.updateListProd)
        self.search.place(x=360, y=120, width=50, height=30)
    ...
    def updateListProd(self):
        try:
            ...
            if brand is not None and model is not None and category is not None:
                ...
                addButton = tk.Button(self, text="Adauga produsele bifate", command=lambda: self.addStock(res,checkList, amountList))
                addButton.place(x=360, y=540, width=150, height=30)
        except:
            pass

    def addStock(self, prods, checkList, amountList):
        nrProd = 0
        isok = 1
        msg = ""
            ...
            if checkList[i].get() == 1:
               ...
               # try:
                if amountList[i].get() != '' and int(amountList[i].get()) > 0:
                    s = "update produse set stock = stock + "+str(amountList[i].get())+" where id_produs = " + str(idProd)
                    self.controller.bdapp.db.cursor.execute(s)
        ...
        for widget in self.prodFrame.winfo_children():
            widget.destroy()
        if isok == 1:
            msg += str(nrProd) + " produse adaugate in stock!"
        else:
